...
fn main() {
    ...
    println!(
        "  AoS: {:?} ({:.1} ns/particle/iter)",
        aos_time,
        aos_time.as_nanos() as f64 / (N as f64 * 1000.0)
    );
    println!(
        "  SoA: {:?} ({:.1} ns/particle/iter)",
        soa_time,
        soa_time.as_nanos() as f64 / (N as f64 * 1000.0)
    );
    println!(
        "  Ratio: {:.2}x",
        aos_time.as_nanos() as f64 / soa_time.as_nanos().max(1) as f64
    );

    // Small density benchmark (N=100)
    let n_small = 100;
    let aos_small: Vec<ParticleAoS> = (0..n_small)
        .map(|i| ParticleAoS {
            position: Vec3 {
                x: (i % 10) as Real * 0.1,
                y: (i / 10) as Real * 0.1,
        ...
        .collect();
    let pos_small: Vec<Vec3> = (0..n_small)
        .map(|i| Vec3 {
            x: (i % 10) as Real * 0.1,
            y: (i / 10) as Real * 0.1,
            z: 0.0,
    ...
    println!(
        "  Ratio: {:.2}x",
        aos_d_time.as_nanos() as f64 / soa_d_time.as_nanos().max(1) as f64
    );
}
